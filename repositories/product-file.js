    ...
    if(search) {
        fs.readFile(DATA_FILE, function (err, data) {
            if (err) {
                ...
                let products = JSON.parse(data);
                // Perform the search
                products = products.filter(row => (search.name ? row.name.toLowerCase().indexOf(search.name.toLowerCase()) >= 0 : true) &&
...
repo.insert = function (newData, resolve, reject) {
    fs.readFile(DATA_FILE, function (err, data){
        if (err) {
            ...
            // Stringify the product array and save to file
